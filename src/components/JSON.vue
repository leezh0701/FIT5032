...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="(ciybt, type) in storeTypes" :key="type">{{ type }}:{{ count }}</li>
      </ul>
      ...
      <ul>
        <li v-for="seller in topSellers" :key="seller">
          {{ seller }}
        </li>
      </ul>
      <p>We operate in:</p>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li
          ...
          :key="author.id"
          :class="{ highlight: isHighlighted(author.id) }"
          :style="getAuthorStyle(author.id)"
        >
          {{ author.name }} ({{ author.birthYear }})
        </li>
      </ul>
...
const highlightedAuthors = ref([2])

const isHighlighted = (authorId) => {
  return highlightedAuthors.value.includes(authorId)
}

const getAuthorStyle = (authorId) => {
  return isHighlighted(authorId) ? { color: 'white', backgroundColor: '#42b883' } : {}
}
</script>
